    ...
    async function setupPayPalButton(sdkInstance: AppSdkInstance) {
        const paypalPaymentSession =
            sdkInstance.createPayPalOneTimePaymentSession(
                enhancedPaymentSessionOptions(
                    (data: OnApproveDataOneTimePayments, orderData: any) => {
                        // ColorConsoleHelper.orange(
                        //     JSON.stringify(orderData, null, 2),
                        // );

                        const vaultId =
                            orderData["capture"]["payment_source"]["paypal"][
                                "attributes"
                            ]["vault"]["id"];
        ...
        const createOrderPromise = createOrderWithVault();
